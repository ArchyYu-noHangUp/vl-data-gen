...
- 多个问题图片或 PDF 页面先分别识别，再按原始页序合并跨文件题干，解决题目跨页时被拆成两题的问题。
- 问题与答案继续独立提取，并在完整问题集合形成后统一匹配。
- 题图统一使用 `题目编号-fig序号.png` 命名，问题正文末尾自动追加对应 Markdown 图片索引。
- 替换题图时自动转换为 PNG，并保持原题图文件名不变。
- 题图定位恢复 DashScope `qwen3.5-27b` 和 0-1000 归一化坐标策略，改善普通扫描图片的裁图稳定性。
- 新增有界并发处理：问题页识别默认 2 并发、题图定位默认 3 并发、答案页识别默认 2 并发；结果始终按原始页码和题目索引排序。
- 新增任务阶段耗时日志，记录上传与 PDF 渲染、问题识别、题图定位、答案识别和任务总耗时。
- 修复 worker 后台进程退出和启动脚本误判进程的问题，并将任务失败详情持久化返回前端。
- 查看结果页题图支持逐图删除，基础信息中的“题目来源”调整为“数据来源”。

本次仅发布源码，Docker 镜像仍为上一已构建版本，待后续明确制作 0.5.0 镜像。
...
```bash
gzip -dc docker_release/vl-data-gen-0.4.4.tar.gz | docker load
docker run -d \
  --name vl-data-gen \
